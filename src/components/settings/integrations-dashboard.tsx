  ...
  async function refreshBitbucket() {
    try {
      const [connRes, oauthRes] = await Promise.all([
        ...
        fetch("/api/integrations/bitbucket"),
      ]);
      ...
      const oauthData = oauthRes.ok
        ? ((await oauthRes.json()) as { oauthConfigured?: boolean })
        : {};

      setBitbucketConn({
        connected: Boolean(connData.connected),
        username: connData.username ?? null,
        workspace: connData.workspace ?? null,
        oauthConfigured: Boolean(oauthData.oauthConfigured),
      });
    } catch {
      /* ignore */
    }
  ...
  const handleConnect = (integrationId: string) => {
    switch (integrationId) {
      ...
      case "gitlab":
        ...
        break;
      case "bitbucket":
        if (bitbucketConn?.oauthConfigured) {
          // OAuth flow
          window.location.href = "/api/integrations/bitbucket/connect?returnTo=%2Fsettings%2Fintegrations";
        } else {
          // App password form
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {bitbucketFormOpen && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center">
          <div className="bg-white rounded-lg p-6 max-w-md w-full">
            <h2 className="text-xl font-bold mb-2 text-gray-900">Connect Bitbucket</h2>
            <p className="text-sm text-slate-600 mb-4">
              Create an app password in your Bitbucket account settings.
            </p>
            <form onSubmit={connectBitbucket} className="space-y-4">
              <div>
                <Label htmlFor="bb-username" className="">Username</Label>
                <Input
                  id="bb-username"
              ...
              <div>
                <Label htmlFor="bb-password" className="">App Password</Label>
                <Input
                  id="bb-password"
                  type="password"
              ...
              <div>
                <Label htmlFor="bb-workspace" className="">Workspace (optional)</Label>
                <Input
                  id="bb-workspace"
      ...
      {azureFormOpen && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center">
          <div className="bg-white rounded-lg p-6 max-w-md w-full">
            <h2 className="text-xl font-bold mb-4 text-gray-900">Connect Azure DevOps</h2>
            <form onSubmit={connectAzure} className="space-y-4">
              <div>
                <Label htmlFor="az-org" className="">Organization</Label>
                <Input
                  id="az-org"
              ...
              <div>
                <Label htmlFor="az-pat" className="">Personal Access Token</Label>
                <Input
                  id="az-pat"
                  type="password"
                  value={azureForm.pat}
